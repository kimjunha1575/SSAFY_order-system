...
app.post('/api/menus', upload.single('file'), async (req, res) => {
  try {
    //multer로 업로드하면 req안에 file객체를 넘겨받는다.
...
app.post('/api/menus/:id/image', upload.single('file'), async (req, res) => {
  try {
    const data = await pool.query(`UPDATE menus SET image_src= ? where id = ?`, [req.file.filename, req.params.id]);
    // console.log(req.file.path);
...
app.get('/api/orders/:id', async (req, res) => {
  try {
    const data = await pool.query(
      `
		SELECT a.id, quantity, request_detail, name, description, b.id
		FROM orders as a
		INNER JOIN menus as b
		ON a.menus_id = b.id
		WHERE a.id =?
...
app.patch('/api/orders/:id', async (req, res) => {
  try {
    ...
    const data = await pool.query(
      ...
      console.log(req.body.request_detail);
    return res.json({
      success: true,
